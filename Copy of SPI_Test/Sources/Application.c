...
#define PAYLOAD_SIZE 5  /* number of payload bytes, 0 to 32 bytes */
#define CHANNEL_NO   45  /* communication channel */
...
void APP_Run(void) {
  ...
  for(;;) {
#if IS_SENDER
    cntr++;
    if (cntr==800) { /* send data every 800 ms */
      cntr = 0;
    ...
    if (cntr>400) { /* not able to send data for 50 ms? */
      LEDR_SetVal();
      LEDG_SetVal();
    }
    ...
    if (isrFlag) { /* check if we have received an interrupt */
      ...
      if (status&RF24_STATUS_TX_DS) { /* data sent interrupt */
        ...
        //LEDG_ClrVal(); /* indicate data has been sent */
      ...
      if (status&RF24_STATUS_MAX_RT) { /* retry timeout interrupt */
        ...
        LEDB_ClrVal(); WAIT1_Waitms(50); LEDB_SetVal();
      }
    }
    WAIT1_Waitms(1);
#else 
    if (isrFlag) { /* interrupt? */
      ...
      LEDB_SetVal();;
     LEDG_ClrVal(); WAIT1_Waitms(50); LEDG_SetVal();// LEDG_Neg(); /* blink green LED to indicate good communication */
      status = RF_GetStatus();
      if (status&RF24_STATUS_RX_DR) { /* data received interrupt */
      ...
      if (cntr>500) { /* blink every 500 ms if not receiving data */
        cntr = 0; /* reset counter */
        LEDG_SetVal(); //LEDG_Off();
        LEDB_ClrVal(); WAIT1_Waitms(50); LEDB_SetVal(); //LEDB_Neg(); /* blink blue to indicate no communication */
      }
