...
char c;
for(;;) {
	ASerialLdd2_ReceiveBlock(&d, &c, 1);
	
	if (c=='r'){RedIO_NegVal();
	Term1_SendStr("red toggled");
	Term1_CRLF();
	}
	
	else if (c=='g'){GreenIO_NegVal();
	Term1_SendStr("green toggled");
	Term1_CRLF();
	}
	
	else if (c=='b'){BlueIO_NegVal();
	Term1_SendStr("blue toggled");
	Term1_CRLF();
	}
}
