...
**            Bus reported device                          : cc3501 mouse
...
byte HIDM1_App_Task(void)
{
  ...
  while (Tx3_Get(&hidCode)==ERR_OK) { /* send everything in the queue */
    /* send message */
...
{
  if (Tx3_Put(((Tx3_ElementType)buttonMask)<<24)!=ERR_OK) { /* put 32bit value into buffer */
    return ERR_FAILED;
  }
  return (Tx3_Put(0)!=ERR_OK); /* send release message */
}
